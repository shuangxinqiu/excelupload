...
$sql = "select * from byddps.{$table} WHERE id = {$id}";
$stmt = $pdo->query($sql);
if ($stmt->rowCount() > 0) {
            ...
            <div class="ibox">
                <div class="ibox float-e-margins">
                    <div class="ibox-title">
                        <h5>编辑<?php echo $table?>id：<?php echo $id?></h5>
...
<!--                            <li class="active"><a href="edit.php?id=--><?php //echo $id ?><!--">基本信息</a>-->
<!--                            </li>-->
<!--                        </ul>-->
                        <form method="post" action="action.php?action=edit&id=<?php echo $item['id']; ?>" class="form-horizontal" >
<!--                            <div class="hr-line-dashed"></div>-->
                            <?php $index=0;foreach ($COLUMN_COMMENT as $i): ?>
                                ...
                                <div class="form-group" <?php if($cf['Visible']==0){echo "style='display:none'";}?>>
                                    ...
                                    <div class="col-sm-10">
                                        <input type="text" class="form-control" name="<?php echo $COLUMN_NAME[$index]; ?>" <?php if($cf['Changeable']==0){echo "disabled='disabled'";}?> value="<?php echo $item[$COLUMN_NAME[$index]]; ?>" >
                                    </div>
                                </div>
                                    ...
                                    <div class="col-sm-10">
                                        <select class="form-control m-b" name="<?php echo $COLUMN_NAME[$index]; ?>" id="<?php echo $COLUMN_NAME[$index]; ?>"  <?php if($cf['Changeable']==0){echo "disabled='disabled'";}?> style="height: 32px;" onchange="tablechanged()">
                                            <?php foreach ($its as $it): ?>
                                                <option value='<?php echo $it[$cf['FromColumn']]?>'<?php if($it[$cf['FromColumn']]==$item[$COLUMN_NAME[$index]]){echo "selected";}?> ><?php echo $it[$cf['FromColumn']].":".$it[$cf['FromColumnShow']]?></option>
                                            <?php endforeach;?>
                                        </select>
                                    </div>
                                </div>
